...
function addEvidenceStrip(presentation, slide, item, y = 5.72) {
  ...
  addText(slide, evidence ? `“${safeText(evidence.quote, 240)}”` : safeText(implication, 240), { x: 0.88, y: y + 0.18, w: 10.2, h: 0.44 }, { fontSize: 15, color: "385B4F", limit: 240 });
...
function deckLayoutGlyph(layout) {
  return { "执行摘要": "◫", "场景对比": "⇄", "洞察证据": "“", "旅程地图": "↗", "策略框架": "⌘", "行动路线图": "→", "研究边界": "◇" }[layout] || "•";
}
...
async function buildDeckScriptPresentation(payload) {
  ...
  presentation.title = safeText(script.title || payload.projectName || "研究洞察报告");
  ...
  slides.forEach((item, index) => {
    ...
    addText(slide, item.title || "洞察页面", { x: 0.62, y: 0.82, w: 11.25, h: 0.68 }, { fontSize: 35, bold: true, color: INK, limit: 82 });
