...
function addEvidenceStrip(presentation, slide, item, y = 5.72) {
  ...
  addText(slide, `“${safeText(evidence.quote, 240)}”`, { x: 0.88, y: y + 0.18, w: 10.2, h: 0.44 }, { fontSize: 15, color: "385B4F", limit: 240 });
  addText(slide, safeText(evidence.document_id || "证据待复核", 80), { x: 11.24, y: y + 0.25, w: 1.15, h: 0.2 }, { fontSize: 9, bold: true, color: GREEN, alignment: "right" });
}
...
function addComparisonLayout(presentation, slide, item) {
  ...
  ["EAF3EE", "F5F0E6"].forEach((color, index) => {
    ...
    addText(slide, scriptBlockText(block, 520), { x: x + 0.28, y: 3.5, w: 5.2, h: 1.48 }, { fontSize: 15, color: "40544C", limit: 520 });
  });
...
function addJourneyLayout(presentation, slide, item) {
  ...
  blocks.forEach((block, index) => {
    ...
    addText(slide, scriptBlockText(block, 260), { x: x - 0.08, y: 4.34, w: 2.65, h: 1.05 }, { fontSize: 13, color: "53665E", limit: 260 });
  });
...
function addFrameworkLayout(presentation, slide, item, roadmap = false) {
  ...
  blocks.forEach((block, index) => {
    ...
    addText(slide, block.heading || `优先级 ${index + 1}`, { x: x + 0.24, y: 2.82, w: width - 0.48, h: 0.58 }, { fontSize: roadmap ? 17 : 20, bold: true, color: INK, limit: 70 });
    addText(slide, scriptBlockText(block, roadmap ? 290 : 380), { x: x + 0.24, y: 3.55, w: width - 0.48, h: 1.4 }, { fontSize: roadmap ? 12 : 14, color: "4C6158", limit: roadmap ? 290 : 380 });
  });
...
function addEvidenceLayout(presentation, slide, item) {
  ...
  blocks.forEach((block, index) => {
    ...
    addText(slide, scriptBlockText(block, 270), { x: 3.18, y, w: 4.45, h: 0.74 }, { fontSize: 13, color: "475B53", limit: 270 });
