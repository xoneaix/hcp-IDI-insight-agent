...
function deckScriptNotes(item = {}) {
  ...
  return [
    ...
    `[Layout]\n${safeText(item.visual_note || item.layout || "按页面信息层级呈现", 600)}`,
...
function addScriptContentBlocks(presentation, slide, item) {
  ...
  const contentBottom = evidence ? 5.48 : 6.44;
  ...
  const gapX = 0.42;
  const gapY = 0.28;
  const blockWidth = columns === 1 ? 12.06 : (12.06 - gapX) / 2;
  const blockHeight = (contentHeight - gapY * (rows - 1)) / rows;
  blocks.forEach((block, index) => {
    const column = index % columns;
    const row = Math.floor(index / columns);
    const x = 0.62 + column * (blockWidth + gapX);
    const y = contentTop + row * (blockHeight + gapY);
    slide.addShape(presentation.ShapeType.line, { x, y, w: 0.44, h: 0, line: { color: index === 0 ? "DFF25B" : "84AB9B", width: 3 } });
    addText(slide, block.heading || `要点 ${index + 1}`, { x, y: y + 0.16, w: blockWidth, h: 0.4 }, { fontSize: 20, bold: true, color: GREEN, limit: 70 });
    addText(slide, block.body || "", { x, y: y + 0.68, w: blockWidth - 0.08, h: Math.max(0.78, blockHeight - 0.72) }, { fontSize: 16, color: "33433D", limit: count > 2 ? 250 : 420 });
  });
  if (!blocks.length) {
    addText(slide, item.takeaway || "本页内容待研究负责人补充。", { x: 0.62, y: 2.28, w: 11.5, h: 2.2 }, { fontSize: 24, color: "33433D" });
  }
  if (evidence) {
    slide.addShape(presentation.ShapeType.roundRect, { x: 0.62, y: 5.72, w: 12.06, h: 0.93, rectRadius: 0.05, fill: { color: "EAF3EE" }, line: { color: "D3E4DB", width: 0.8 } });
    addText(slide, `“${safeText(evidence.quote, 260)}”`, { x: 0.88, y: 5.92, w: 10.35, h: 0.48 }, { fontSize: 16, color: "385B4F", limit: 260 });
    addText(slide, safeText(evidence.document_id || "证据待复核", 80), { x: 11.36, y: 5.98, w: 1.03, h: 0.24 }, { fontSize: 9, bold: true, color: GREEN, alignment: "right" });
  }
}
...
async function buildDeckScriptPresentation(payload) {
  ...
  presentation.title = safeText(script.title || payload.projectName || "综合洞察与策略 Deck");
  ...
  slides.forEach((item, index) => {
    ...
    if (cover) {
      ...
      addText(slide, item.title || script.title || payload.projectName, { x: 0.72, y: 2.05, w: 11.2, h: 1.54 }, { fontSize: 50, bold: true, color: "FFFFFF", limit: 76 });
    ...
    addText(slide, item.title || "洞察页面", { x: 0.62, y: 0.82, w: 12.0, h: 0.68 }, { fontSize: 35, bold: true, color: INK, limit: 82 });
    addText(slide, item.takeaway || "", { x: 0.62, y: 1.55, w: 11.9, h: 0.42 }, { fontSize: 18, color: "4E665D", limit: 150 });
