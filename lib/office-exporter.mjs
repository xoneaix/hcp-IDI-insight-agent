...
function addFrameworkLayout(presentation, slide, item, roadmap = false) {
  ...
  blocks.forEach((block, index) => {
    ...
    addText(slide, scriptBlockText(block, roadmap ? 190 : 250), { x: x + 0.24, y: 3.55, w: width - 0.48, h: 1.4 }, { fontSize: 16, color: "4C6158", limit: roadmap ? 190 : 250 });
  });
...
function addPriorityLayout(presentation, slide, item) {
  ...
  blocks.forEach((block, index) => {
    ...
    addText(slide, block.heading || `机会 ${index + 1}`, { x: pos.x + 0.18, y: pos.y + 0.45, w: 2.38, h: 0.42 }, { fontSize: 16, bold: true, color: "FFFFFF", limit: 42 });
  });
