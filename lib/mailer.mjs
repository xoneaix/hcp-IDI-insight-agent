...
export async function sendAdminPasswordResetEmail({ email, temporaryPassword }) {
  ...
  const htmlContent = `<div style="font-family:Arial,'Microsoft YaHei',sans-serif;max-width:620px;margin:auto;color:#16201e"><div style="background:#174b3e;padding:28px;border-radius:16px 16px 0 0;color:white"><div style="font-size:13px;letter-spacing:2px;color:#dff25b">MEDVOICE INSIGHT · SECURITY</div><h1 style="margin:8px 0 0">登录密码已重置</h1></div><div style="padding:30px;border:1px solid #dfe6e2;border-top:0;border-radius:0 0 16px 16px"><p>你好，管理员刚刚为你的 MedVoice Insight 账号生成了新的临时密码。此前设置的密码及旧临时密码均已失效。</p><div style="background:#f1f5f3;padding:18px;border-radius:10px;margin:22px 0"><div><strong>登录邮箱：</strong>${escapeHtml(email)}</div><div style="margin-top:10px"><strong>新临时密码：</strong><code style="font-size:16px;color:#245c4d">${escapeHtml(temporaryPassword)}</code></div></div><p><a href="${escapeHtml(appUrl)}" style="display:inline-block;background:#245c4d;color:white;text-decoration:none;padding:12px 20px;border-radius:8px;font-weight:bold">使用新临时密码登录</a></p><p style="color:#71807a;font-size:13px">登录后系统会要求设置至少 12 位的新密码。本次重置已注销该账号的其他登录会话；如非本人申请，请立即联系管理员。</p></div></div>`;
  return deliverEmail({ email, subject, htmlContent });
}
