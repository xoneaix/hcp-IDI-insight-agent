...
async function deliverEmail({ email, subject, htmlContent }) {
...
export async function sendAccessApprovedEmail({ email, temporaryPassword }) {
  ...
  return deliverEmail({ email, subject, htmlContent });
}

export async function sendAdminPasswordResetEmail({ email, temporaryPassword }) {
  const appUrl = process.env.PUBLIC_APP_URL || "https://medvoice-insight-agent.onrender.com/";
  const issuedAt = new Intl.DateTimeFormat("zh-CN", {
    timeZone: "Asia/Shanghai",
    dateStyle: "long",
    timeStyle: "medium"
  }).format(new Date());
  const subject = "MedVoice Insight 密码已由管理员重置";
  const htmlContent = `<div style="font-family:Arial,'Microsoft YaHei',sans-serif;max-width:620px;margin:auto;color:#16201e"><div style="background:#174b3e;padding:28px;border-radius:16px 16px 0 0;color:white"><div style="font-size:13px;letter-spacing:2px;color:#dff25b">MEDVOICE INSIGHT · SECURITY</div><h1 style="margin:8px 0 0">登录密码已重置</h1></div><div style="padding:30px;border:1px solid #dfe6e2;border-top:0;border-radius:0 0 16px 16px"><p>你好，管理员刚刚为你的 MedVoice Insight 账号生成了新的临时密码。此前设置的密码及旧临时密码均已失效。</p><div style="background:#f1f5f3;padding:18px;border-radius:10px;margin:22px 0"><div><strong>登录邮箱：</strong>${escapeHtml(email)}</div><div style="margin-top:10px"><strong>新临时密码：</strong><code style="font-size:16px;color:#245c4d">${escapeHtml(temporaryPassword)}</code></div><div style="margin-top:10px;color:#71807a;font-size:13px"><strong>生成时间：</strong>${escapeHtml(issuedAt)}（北京时间）</div></div><p><a href="${escapeHtml(appUrl)}" style="display:inline-block;background:#245c4d;color:white;text-decoration:none;padding:12px 20px;border-radius:8px;font-weight:bold">使用新临时密码登录</a></p><p style="color:#71807a;font-size:13px">请只使用生成时间最新的一封密码邮件。登录后系统会要求设置至少 12 位的新密码。本次重置已注销该账号的其他登录会话；如非本人申请，请立即联系管理员。</p></div></div>`;
  return deliverEmail({ email, subject, htmlContent });
}

export async function sendPasswordResetLinkEmail({ email, resetUrl, expiresMinutes = 30 }) {
  const subject = "重置你的 MedVoice Insight 密码";
  const htmlContent = `<div style="font-family:Arial,'Microsoft YaHei',sans-serif;max-width:620px;margin:auto;color:#16201e"><div style="background:#174b3e;padding:28px;border-radius:16px 16px 0 0;color:white"><div style="font-size:13px;letter-spacing:2px;color:#dff25b">MEDVOICE INSIGHT · SECURITY</div><h1 style="margin:8px 0 0">验证邮箱并重置密码</h1></div><div style="padding:30px;border:1px solid #dfe6e2;border-top:0;border-radius:0 0 16px 16px"><p>我们收到了 ${escapeHtml(email)} 的密码重置申请。</p><p><a href="${escapeHtml(resetUrl)}" style="display:inline-block;background:#245c4d;color:white;text-decoration:none;padding:13px 22px;border-radius:8px;font-weight:bold">验证邮箱并设置新密码</a></p><p style="color:#596762">此链接将在 ${escapeHtml(expiresMinutes)} 分钟后失效，且只能使用一次。完成重置后，该账号的其他登录会话会自动注销。</p><div style="word-break:break-all;background:#f1f5f3;padding:14px;border-radius:8px;color:#71807a;font-size:12px">${escapeHtml(resetUrl)}</div><p style="color:#71807a;font-size:13px">如果不是你发起的申请，可以忽略本邮件，原密码不会因此改变。</p></div></div>`;
  return deliverEmail({ email, subject, htmlContent });
}
