...
export class AuthStore {
  ...
  async resetPasswordById(userId) {
    const user = this.db.prepare("SELECT * FROM users WHERE id=? AND active=1").get(Number(userId));
    ...
    this.db.prepare("UPDATE users SET password_hash=?, must_change_password=1, updated_at=? WHERE id=?")
      .run(await hashPassword(password), now, user.id);
    this.db.prepare("DELETE FROM sessions WHERE user_id=?").run(user.id);
    this.db.prepare("DELETE FROM password_reset_tokens WHERE user_id=?").run(user.id);
    return { email: user.email, temporaryPassword: password };
  }

  createPasswordReset(email, minutes = 30) {
    email = normalizeEmail(email);
    if (!this.isEmailAllowed(email)) return null;
    const user = this.db.prepare("SELECT id,email FROM users WHERE email=? AND active=1").get(email);
    if (!user) return null;
    const token = randomBytes(32).toString("base64url");
    const now = new Date();
    const expires = new Date(now.getTime() + Math.max(10, Number(minutes) || 30) * 60_000);
    this.db.prepare("DELETE FROM password_reset_tokens WHERE user_id=? OR expires_at<?").run(user.id, now.toISOString());
    this.db.prepare("INSERT INTO password_reset_tokens(user_id,token_hash,expires_at,created_at) VALUES(?,?,?,?)")
      .run(user.id, sessionHash(token), expires.toISOString(), now.toISOString());
    return { email: user.email, token, expires };
  }

  async resetPasswordWithToken(token, newPassword) {
    if (String(newPassword || "").length < 12) throw new Error("新密码至少需要 12 位");
    const tokenHash = sessionHash(String(token || ""));
    const now = new Date().toISOString();
    const record = this.db.prepare(`SELECT p.id,p.user_id,u.email
      FROM password_reset_tokens p
      JOIN users u ON u.id=p.user_id
      WHERE p.token_hash=? AND p.used_at IS NULL AND p.expires_at>? AND u.active=1`).get(tokenHash, now);
    ...
    try {
      ...
      this.db.prepare("UPDATE password_reset_tokens SET used_at=? WHERE id=?").run(now, record.id);
      this.db.prepare("DELETE FROM sessions WHERE user_id=?").run(record.user_id);
      this.db.prepare("DELETE FROM password_reset_tokens WHERE user_id=? AND id<>?").run(record.user_id, record.id);
      this.db.exec("COMMIT");
    } catch (error) {
  ...
  async addUser(email, role = "user") {
    ...
    if (existing) {
      ...
      this.db.prepare("DELETE FROM password_reset_tokens WHERE user_id=?").run(existing.id);
    } else {
      this.db.prepare("INSERT INTO users(email,password_hash,role,active,must_change_password,created_at,updated_at) VALUES(?,?,?,?,?,?,?)")
