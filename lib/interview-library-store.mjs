...
function normalizePatch(patch = {}) {
  ...
  const name = redactProductNames(patch.name).slice(0, 240);
  return {
    projectId: String(patch.projectId || patch.project_id || "default").replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 80) || "default",
    projectName: redactProductNames(patch.projectName || patch.project_name || "未命名访谈项目").slice(0, 80),
    clientId,
    name,
    ...
    progressText: redactProductNames(patch.progressText).slice(0, 500),
    error: redactProductNames(patch.error).slice(0, 2000),
    text: redactProductNames(patch.text),
    draftText: redactProductNames(patch.draftText),
    roleResult: redactProductReferences(patch.roleResult) || null
  };
}
...
function publicItem(row) {
  ...
  return {
    ...
    projectName: redactProductNames(row.project_name || "未命名访谈项目"),
    serverId: String(row.id),
    id: row.client_id,
    name: redactProductNames(row.name),
    ...
    status: row.status,
    progressText: redactProductNames(row.progress_text || ""),
    error: redactProductNames(row.error || ""),
    text: redactProductNames(row.transcript_text || ""),
    draftText: redactProductNames(row.draft_text || ""),
    roleResult: redactProductReferences(roleResult),
    fileName: redactProductNames(row.file_name),
...
function redactedStoredRoleResult(value) {
  if (!value) return null;
  if (typeof value === "string") {
    try {
      return JSON.stringify(redactProductReferences(JSON.parse(value)));
    } catch {
      return redactProductNames(value);
    }
  }
  return JSON.stringify(redactProductReferences(value));
}
...
export class PostgresInterviewLibraryStore {
  ...
  async redactLegacyProductNames() {
    const result = await this.pool.query(`
      SELECT id, name, project_name, progress_text, error, transcript_text, draft_text, role_result, file_name
      FROM interview_assets
    `);
    for (const row of result.rows) {
      if (!containsProductBrandName(JSON.stringify(row))) continue;
      await this.pool.query(
        `UPDATE interview_assets SET
           name=$2, project_name=$3, progress_text=$4, error=$5, transcript_text=$6,
           draft_text=$7, role_result=$8::jsonb, file_name=$9, updated_at=NOW()
         WHERE id=$1`,
        [
          row.id,
          redactProductNames(row.name),
          redactProductNames(row.project_name),
          redactProductNames(row.progress_text),
          redactProductNames(row.error),
          redactProductNames(row.transcript_text),
          redactProductNames(row.draft_text),
          redactedStoredRoleResult(row.role_result),
          redactProductNames(row.file_name)
        ]
      );
    }
  }
  ...
  async createItem(userId, id, patch, file) {
    ...
    const result = await this.pool.query(
      `INSERT INTO interview_assets(id,user_id,client_id,name,project_id,project_name,respondent_type,source,recorded_at,duration_seconds,status,progress_text,error,transcript_text,draft_text,role_result,file_name,mime_type,file_size,storage_path,has_file)
       ...
       RETURNING *`,
      [id, userId, item.clientId, item.name || redactProductNames(file.fileName), item.projectId, item.projectName, item.type, item.source, item.recordedAt, item.durationSeconds, item.status, item.progressText, item.error, item.text, item.draftText, item.roleResult ? JSON.stringify(item.roleResult) : null, redactProductNames(file.fileName), file.mimeType, file.fileSize, file.storagePath]
    );
  ...
  async deleteProject(userId, projectId) {
    const result = await this.pool.query("DELETE FROM interview_assets WHERE user_id=$1 AND project_id=$2 RETURNING storage_path", [userId, projectId]);
    return result.rows.map((row) => row.storage_path);
  }
}
...
export class SqliteInterviewLibraryStore {
  ...
  redactLegacyProductNames() {
    ...
    const update = this.db.prepare(`
      UPDATE interview_assets SET
        name=?, project_name=?, progress_text=?, error=?, transcript_text=?, draft_text=?,
        role_result=?, file_name=?, updated_at=?
      WHERE id=?
    `);
  ...
  async deleteProject(userId, projectId) {
    const rows = this.db.prepare("SELECT storage_path FROM interview_assets WHERE user_id=? AND project_id=?").all(userId, projectId);
    this.db.prepare("DELETE FROM interview_assets WHERE user_id=? AND project_id=?").run(userId, projectId);
    return rows.map((row) => row.storage_path);
  }
}
