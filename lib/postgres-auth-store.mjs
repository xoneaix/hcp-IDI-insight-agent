...
import { hashPassword, isCompanyEmail, normalizeEmail, temporaryPassword, verifyPassword } from "./auth-store.mjs";
...
export class PostgresAuthStore {
  ...
  async authenticate(email, password) {
    const result = await this.pool.query("SELECT * FROM users WHERE email=$1 AND active=TRUE", [normalizeEmail(email)]);
  ...
  createSession(userId, hours = 12) {
    ...
    this.sessions.set(token, { user: this.userCache.get(Number(userId)), expires });
  ...
  deleteSession(token) { if (token) this.sessions.delete(token); }
