...
import { hashPassword, isCompanyEmail, normalizeEmail, sessionHash, temporaryPassword, verifyPassword } from "./auth-store.mjs";
...
export class PostgresAuthStore {
  ...
  async sessionUser(token) {
    ...
    if (session && session.expires > new Date() && session.user) return session.user;
